...
const derivationPath = "m/44'/0'/0'/0/0" // BIP44 derivation path
...
var btcMap = map[string]bitcoin{
	"btc":    {name: "bitcoin", xpub: 0x00, xpriv: 0x80, bip39: false, isSegWit: false},
	"bip39":  {name: "bitcoin[bip39]", xpub: 0x00, xpriv: 0x80, bip39: true, isSegWit: false},
	"segwit": {name: "bitcoin[segwit]", xpub: 0x05, xpriv: 0x80, bip39: false, isSegWit: true},
}

func (btc bitcoin) getParams() *chaincfg.Params {
	param := &chaincfg.MainNetParams // Always use mainnet
	if btc.isSegWit {
		// Mainnet SegWit parameters
		param.PubKeyHashAddrID = 0x6F // SegWit P2WPKH address prefix (mainnet)
...
func (btc bitcoin) createPrivateKey() (*btcutil.WIF, error) {
	if btc.bip39 {
		// Generate BIP39 mnemonic, and use it to create a private key
		...
		// Correct private key for mainnet SegWit
...
// GetAddress will generate either a P2PKH or P2WPKH address depending on the SegWit flag
func (btc bitcoin) getAddress(wif *btcutil.WIF) (btcutil.Address, error) {
	pubKey := wif.PrivKey.PubKey()

	// If it's SegWit, generate a P2WPKH address (starts with 'bc1')
	if btc.isSegWit {
...
func generateBIP39Mnemonic() (string, error) {
	...
	for i := 0; i < 24; i++ {
		...
		randomIndex := int(index[0]) + int(index[1])*256
		randomIndex = randomIndex % 2048
		mnemonic = append(mnemonic, wordlist[randomIndex])
	}

	return strings.Join(mnemonic, " "), nil
}

// GenerateKeys generates a keypair, including the address based on the network type (SegWit or legacy)
func (btc bitcoin) GenerateKeys() (*KeyPair, error) {
	...
	// If it's a BIP39 key, generate mnemonic and derivation path
	if btc.bip39 {
		mnemonic, err := generateBIP39Mnemonic()
		if err != nil {
			return nil, err
